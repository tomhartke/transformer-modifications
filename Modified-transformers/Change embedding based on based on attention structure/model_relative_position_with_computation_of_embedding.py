class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        att_loc = torch.cat((att_loc_rel, att_loc_std, att_loc_orig), dim=-1) # (B, T, n_head*2 + 1)
...
class MLP_loc(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        n_params_att_loc = (config.n_head*2 + 1)
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + x_vals * (1.0 + self.mlp_scale(x_att_loc)) + self.mlp_bias(x_att_loc)
        x = x + self.mlp(self.ln_2(x)) + self.mlp_loc(x_att_loc)
        return x
